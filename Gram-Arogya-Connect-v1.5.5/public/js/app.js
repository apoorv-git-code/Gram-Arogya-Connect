...
async function ensureDoctorShareQr() {
  const box = $('#abha-share-qr');
  if (!box || !currentUser || currentUser.role !== 'patient' || box.dataset.loaded === 'true') return;
  box.dataset.loaded = 'true';
  try {
    const response = await fetch('/api/v1/profile/share', { method: 'POST', credentials: 'same-origin' });
    const payload = await response.json();
    if (!response.ok || !payload.data?.qrSvg) throw new Error(payload.message || 'QR unavailable');
    box.innerHTML = `${payload.data.qrSvg}<small>Doctor scan</small>`;
    box.title = `Scan to view the latest shared record. Link expires ${new Date(payload.data.expiresAt).toLocaleDateString()}.`;
  } catch (error) {
    box.classList.add('is-error');
    box.innerHTML = '<span class="qr-loading">QR unavailable</span><small>Try again online</small>';
