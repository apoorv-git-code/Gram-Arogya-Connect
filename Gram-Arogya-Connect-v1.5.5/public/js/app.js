...
function bindAccountUI(){
  ...
  $('#auth-switch')?.addEventListener('click',()=>{authMode=authMode==='login'?'register':'login'; const reg=authMode==='register'; $('#auth-name-field').hidden=!reg; $('#auth-role-field').hidden=!reg; $('#auth-title').textContent=reg?'Create your account':'Welcome back'; $('#auth-subtitle').textContent=reg?'Your health information will be linked to this account.':'Log in to continue to your health space.'; $('#auth-submit').textContent=reg?'Create account':'Log in'; $('#auth-switch').textContent=reg?'Already have an account? Log in':'New user? Create account'; $('#auth-password').autocomplete=reg?'new-password':'current-password'; $('#auth-error').textContent='';});
