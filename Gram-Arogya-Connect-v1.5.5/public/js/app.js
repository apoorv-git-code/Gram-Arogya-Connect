...
function navigate(view, updateHash = true) {
  ...
  if (view === 'records') setTimeout(() => ensureDoctorShareQr(true), 0);
...
async function ensureDoctorShareQr(force = false) {
  const box = $('#abha-share-qr');
  if (!box || !currentUser || currentUser.role !== 'patient') return;
  if (!force && box.dataset.loaded === 'true') return;
  if (box.dataset.loading === 'true') return;
  box.dataset.loading = 'true';
  box.classList.remove('is-error');
  box.innerHTML = '<span class="qr-loading">Creating secure QR…</span><small>Doctor scan</small>';
  try {
    const response = await fetch('/api/v1/profile/share', {
      method: 'POST',
    ...
    const payload = await response.json();
    if (!response.ok || !payload.data?.qrSvg || !payload.data?.shareUrl) {
      throw new Error(payload.message || 'QR unavailable');
    }
    box.dataset.loaded = 'true';
    box.innerHTML = `${payload.data.qrSvg}<small>Scan by doctor</small><a class="qr-test-link" href="${payload.data.shareUrl}" target="_blank" rel="noopener">Test QR link</a>`;
    box.title = `Scan to view the latest shared record. Link expires ${new Date(payload.data.expiresAt).toLocaleDateString()}.`;
  } catch (error) {
    box.classList.add('is-error');
    box.dataset.loaded = 'false';
    box.innerHTML = `<span class="qr-loading">QR unavailable</span><small>${String(error.message || 'Try again online')}</small><button class="qr-retry" type="button">Retry</button>`;
    const retry = box.querySelector('.qr-retry');
    if (retry) retry.addEventListener('click', () => ensureDoctorShareQr(true));
  } finally {
