...
const viewMeta = {
  home: ['YOUR HEALTH SPACE', 'Good morning'],
...
const phraseBook = {
  ...
  'Preparing your secure health space…': ['आपका सुरक्षित स्वास्थ्य क्षेत्र तैयार हो रहा है…', 'तुमचे सुरक्षित आरोग्य क्षेत्र तयार होत आहे…'],
  ...
  'YOUR HEALTH SPACE': ['आपका स्वास्थ्य क्षेत्र', 'तुमचे आरोग्य क्षेत्र'],
  ...
  'Latest vitals': ['नवीनतम माप', 'नवीनतम मोजमाप'],
  'See history': ['इतिहास देखें', 'इतिहास पाहा'],
  'Blood pressure': ['रक्तचाप', 'रक्तदाब'],
  'Watch': ['ध्यान दें', 'लक्ष द्या'],
  'Oxygen level': ['ऑक्सीजन स्तर', 'ऑक्सिजन पातळी'],
...
async function initAccount() {
  ...
  // Only the login ID and role may be remembered locally; passwords are never stored by the app.
  const remembered = loadJSON('gac-login-hint', null);
  if (remembered?.loginId) $('#auth-login').value = remembered.loginId;
  if (remembered?.role) { const radio=document.querySelector(`input[name="auth-role"][value="${remembered.role}"]`); if(radio) radio.checked=true; }
  $('#auth-remember').checked = Boolean(remembered?.loginId);
  showAuth();
}
function bindAccountUI(){
  $('#auth-form')?.addEventListener('submit', submitAuth);
  $('#auth-switch')?.addEventListener('click',()=>{authMode=authMode==='login'?'register':'login'; const reg=authMode==='register'; $('#auth-name-field').hidden=!reg; $('#auth-title').textContent=reg?'Create your account':'Welcome back'; $('#auth-subtitle').textContent=reg?'Your health information will be linked to this account.':'Log in to continue to your health space.'; $('#auth-submit').textContent=reg?'Create account':'Log in'; $('#auth-switch').textContent=reg?'Already have an account? Log in':'New user? Create account'; $('#auth-password').autocomplete=reg?'new-password':'current-password'; $('#auth-error').textContent='';});
...
async function submitAuth(event){
  ...
  const body={loginId:$('#auth-login').value.trim(),password:$('#auth-password').value,remember:$('#auth-remember').checked,role:document.querySelector('input[name="auth-role"]:checked')?.value||'patient'};
  ...
  try{
    ...
    if(body.remember) saveJSON('gac-login-hint',{loginId:body.loginId,role:body.role}); else localStorage.removeItem('gac-login-hint');
    if(mode==='register'){
      ...
      currentUser=null; authMode='login'; $('#auth-name-field').hidden=true; $('#auth-title').textContent='Account created';
      $('#auth-subtitle').textContent='Log in with the account you just created to enter your health space.'; $('#auth-submit').textContent='Log in';
